...
function showSlides(n) {
    var slides = document.getElementsByClassName("mySlides"); // this takes all elements with the class name "mySlides" and stores them
    // in the variable array "slides"
    ...
    if (n < 1) {slideIndex = slides.length}; // if n (the number passed into the function) is less than 1, the
    // the slideIndex is set to the length of the array "slides"
...
document.addEventListener("click", function(event){
    // here we state that if the click happens on the cancel button OR anywhere that is not the contact form AND the click
    // does not happen on any element with the contact class then call the closeForm() function
